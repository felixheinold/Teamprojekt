...
export const useBackendUserContext = (): BackendUserContextType => {
    ...
    if (!context){
        throw new Error("useBackendUserContext has to be with a BackendUserProvider. Error in BackendUserContext.tsx")
    }
    return context;
}
...
export const BackendUserProvider = ({ children } : {children: React.ReactNode}) => {
    ...
    const flushUser = async () => {
        ...
        if (user && hasUntrackedChanges){
            try{
                ...
                console.error("Error while saving user in Firestore. Error message from BackendUserContext.tsx");
            }   
        }
    ...
    return (
    <BackendUserContext.Provider value={{ user, setUser, flushUser, untrackedChanges, hasUntrackedChanges }}>
      {children}
